...
<?php $url_esc = str_replace ('/','z_z',uri_string());
...
<?php if ( $sesion["ses_usuario_id"] == $usuario->id )
	echo '  '.anchor('evalua/emitidas/'.$usuario->nick,'<button class="btn btn-default">Emitidas</button>'); ?>
	
<?php if ($sesion["ses_socio"]>0 && count($evaluaciones)==20)
		echo '  '.anchor('evalua/recibidasantiguas/'.$usuario->nick,'<button class="btn btn-default">Antiguas</button>'); 
?>
